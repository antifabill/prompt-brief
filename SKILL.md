...
In this mode, keep the original three-C interview method. Do not skip Context, Constraints, or Composition. The difference is the compiled execution payload: make it lean, outcome-first, and tuned for GPT-5.5 rather than preserving every human-facing note.
...
- `current-agent` or `fresh-agent` with GPT-5.5 tuning mode: create a concise `GPT-5.5 Tuned Execution Payload` from the approved human brief and curated context snapshot. Preserve the user's intent and hard constraints, but compress the working instructions into the GPT-5.5 payload shape.
...
- Explicit invocation should use the real skill name, for example: `$prompt-brief create an executable brief for: <rough task>`.
