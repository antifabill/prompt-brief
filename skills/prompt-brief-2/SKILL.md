---
name: prompt-brief-2
description: Turn a rough user request into an executable agent brief using an interview-first workflow with guided questions. Use when the user asks to improve or engineer a prompt for an AI agent, wants help turning a vague task into a runnable brief, wants multiple-choice guidance for missing context or verification criteria, or wants approved briefs logged into a reusable prompt library without affecting the original prompt-brief skill.
...
- When the user does not naturally provide context, constraints, or composition, translate their rough request into those categories yourself and then ask for the missing pieces before drafting the final card when those answers would materially improve it.
- Default to agent-style briefs, not generic chatbot prompt polishing.
- Default to an interview-first prompt artifact: ask guiding questions when material user-owned context is missing; otherwise return the prompt card.
...
- Your first deliverable is always a short question round when material user-owned context is missing; otherwise it is the structured prompt-brief artifact. It is never the finished work product from the rough task.
...
- `$prompt-brief-2 use a subagent to engineer this brief: ...`
...
The prompt-engineering pass must return either a short question round or a structured artifact with these sections:
...
- If no question is needed, return `Next Questions: none` and include a one-sentence reason in `Remaining Gaps / Assumptions` explaining why the brief is ready without more user input.
...
- When asking two or more questions, make at least half of them multiple-choice unless the missing information genuinely cannot be represented as choices.
- For single-question rounds, prefer multiple-choice by default.
...
Before writing the final engineered brief, classify every remaining gap as one of:

- `discoverable`: Codex can inspect files, docs, or workspace state to answer it.
- `assumable`: the answer is low-risk and can be named explicitly as an assumption.
- `user-owned/material`: the answer depends on user preference, priority, audience, scope, approval, external ownership, or another decision that would materially change the brief.

If any `user-owned/material` gap exists, do not return the final prompt-brief card yet. Ask 1-3 guiding questions first using the interview rules above.

Only return `Next Questions: none` when no `user-owned/material` gaps remain. In that case, briefly state why the remaining gaps are discoverable or safely assumable.
...
- the remaining unknowns are only `discoverable` or `assumable` gaps that can be named explicitly in the final brief.
...
### 10. Save Approved Briefs To The Prompt Library
...
- Explicit invocation should use the real skill name, for example: `$prompt-brief-2 draft a stronger agent brief for refactoring the auth flow`.
- A good explicit invocation for this skill is: `$prompt-brief-2 create an executable brief for: <rough task>`.
